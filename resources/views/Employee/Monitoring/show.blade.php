...
@section('content')
            ...
            <div class="player-info-block">
                ...
                <div class="player-meta">
                    ...
                    <div class="meta-badges">
                        ...
                        <button type="button" class="badge-item" onclick="openModal('renewSubModal')"
                            style="cursor:pointer; color:#c9a961; background: rgba(201,169,97,0.10); border-color: rgba(201,169,97,0.25);">
                            <i class="fas fa-rotate" style="margin-left:5px;"></i> تجديد الاشتراك
                        </button>
                    </div>
                </div>
            </div>
    ...
    <div id="renewSubModal" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <h4><i class="fas fa-rotate" style="color: var(--gold);"></i> تجديد اشتراك اللاعب</h4>
                <span class="close-modal" onclick="closeModal('renewSubModal')">&times;</span>
            </div>
            <form action="{{ route('employee.monitoring.renew-subscription', $player->id) }}" method="POST">
                @csrf
                <div class="modal-body">
                    @if ($player->subscription)
                        <p style="font-size: 12.5px; color: var(--muted); margin-bottom: 14px;">
                            الباقة الحالية: <strong
                                style="color: var(--text);">{{ $player->subscription->plan_name }}</strong>
                        </p>
                    @endif

                    <div class="field-group">
                        <label class="field-label">اختر الباقة الجديدة (يمكن تغيير نوع الاشتراك)</label>
                        <select name="plan_type_id" class="field-input" required>
                            <option value="">-- اختر الباقة --</option>
                            @forelse ($planTypes as $planType)
                                <option value="{{ $planType->id }}"
                                    {{ optional($player->subscription)->plan_type_id == $planType->id ? 'selected' : '' }}>
                    ...
                    <p style="font-size: 11.5px; color: var(--muted); margin-top: -6px; margin-bottom: 16px;">
                        سيبدأ الاشتراك الجديد من تاريخ اليوم، بغض النظر عن الباقة المختارة.
                    </p>

                    <button type="submit" class="btn-submit">تأكيد التجديد</button>
                </div>
            </form>
        </div>
    </div>
