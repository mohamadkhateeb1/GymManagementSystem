    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-submit {
            ...
            cursor: pointer; box-shadow: 0 4px 18px rgba(251, 191, 36, 0.28); transition: opacity 0.2s, transform 0.2s;
        }
        .btn-submit:hover { opacity: .9; transform: translateY(-1px); box-shadow: 0 6px 24px rgba(251, 191, 36, 0.38); }
    </style>
</head>
<body>
    <div class="wrapper">
        <x-flash-message />
        <div class="header">
            <div class="header-title">تعديل الموظف: {{ $employee->username }}</div>
        ...
        <div class="card">
            <form action="{{ route('employees.update', $employee->id) }}" method="POST">
                ...
                <button type="submit" class="btn-submit">تحديث بيانات الموظف</button>
            </form>
        </div>
    </div>
</body>
