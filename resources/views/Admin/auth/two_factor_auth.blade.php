        ...
        <div class="two-factor-card">
            ...
            <div class="two-factor-body">
                ...
                @if ($user->two_factor_secret && $user->two_factor_confirmed_at)
                    ...
                    {{-- ✅ الحالة 3: مفعّلة ومؤكَّدة بالكامل — لا نعرض الـ QR/السر مجدداً لأي سبب أمني --}}
                    ...
                    <div class="recovery-section" style="margin-top: 16px;">
                ...
                @elseif (!$user->two_factor_secret)
                    ...
                    <form action="{{ route('admin.two-factor.enable') }}" method="POST">
                        ...
                        <button type="submit" class="two-factor-btn two-factor-btn-primary">
                ...
                @else
                    ...
                    {{-- 🆕 خطوة التأكيد — إجبارية حتى يفعّل لارافيل التحقق فعلياً عند الدخول.
                     بدون هالخطوة، القيمة two_factor_confirmed_at تضل NULL للأبد،
                     والنظام ما بيطلب أي رمز وقت تسجيل الدخول إطلاقاً. --}}
                    <div class="two-factor-alert two-factor-alert-info" style="margin-top: 22px;">
                        <i class="fas fa-circle-exclamation"></i>
                        <span>
                            <strong>خطوة أخيرة إجبارية:</strong> امسح الرمز أعلاه بتطبيق المصادقة،
                            وأدخل الرمز المكوّن من 6 أرقام اللي بيظهرلك بالتطبيق هون تحت،
                            وإلا **لن يُطلب منك أي رمز عند تسجيل الدخول لاحقاً**.
                        </span>
                    </div>
                    ...
                    <form action="{{ route('admin.two-factor.confirm') }}" method="POST" style="margin-top: 14px;">
                        ...
                        <div class="form-group" style="margin-bottom: 14px;">
                            <label for="two_factor_code"
                                style="display:block; margin-bottom:7px; font-weight:700; color: var(--text, #202631); font-size: 13.5px;">
                                رمز التأكيد (6 أرقام)
                            </label>
                            <input type="text" name="code" id="two_factor_code" inputmode="numeric"
                                autocomplete="one-time-code" maxlength="6" placeholder="000000" required
                                style="width:100%; height:46px; padding:0 14px; border-radius:10px; border:1px solid var(--border, #dfe4eb); font-size:18px; letter-spacing:6px; text-align:center; font-weight:800;">
                            @error('code')
                ...
                <a href="{{ route('admin.dashboard') }}" class="two-factor-back">
